...
//    protected static final String FREQUENT_PATTERN_PATH = "freqPatternPath";
    ...
    public int run(String[] strings) throws Exception {
        addInputOption();
        addOutputOption();
        addOption("itemdistribute", "ids", "item distribute", "");
        ...
        Path frequentPatterns = new Path(getInputPath(), CopyFpGrowth.FREQUENT_PATTERNS);
        Path itemDistributePath = new Path(getOption("itemdistribute"));

        // delete item counter file
        try {
            HadoopUtil.delete(getConf(), itemDistributePath);
        } catch (IOException e) {
            e.printStackTrace();
        }
